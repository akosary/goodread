...
const photoValidation = Joi.string().uri({
  scheme: [
    'http',
    'https'
  ]
    ...
    createBook (req, res) {
      ...
      const { error } = photoValidation.validate(photo);
        ...
        try {
          ...
          {
            return res.status(400).json({ error: 'CategoryId is requied and must be ObjectId ' });
          }
          else if(!isValidObjectId(authorId)){

            return res.status(400).json({ error: 'AuthorId is requied and must be ObjectId ' });
          }
